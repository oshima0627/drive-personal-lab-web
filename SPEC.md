# 運転不安セルフ診断アプリ — Drive Personal Lab
## Webアプリ版 設計仕様書 v5.0
### （Androidアプリ版 v6.0 対応）
...
- スコアが高い分類（60点以上）：共感コメント＋詳細説明を表示
- スコアが低い分類（60点未満）：「この分野はあまり気になっていないようです」を表示
...
| 4分類すべて60点以上 | 総合複合タイプ | `all` |
| 3分類が60点以上 | 3分類複合タイプ（4パターン） | `kn_sk_ex` / `kn_sk_en` / `kn_ex_en` / `sk_ex_en` |
| 2分類が60点以上 | 2分類複合タイプ（6パターン） | `kn_sk` / `kn_ex` / `kn_en` / `sk_ex` / `sk_en` / `ex_en` |
| 1分類が60点以上 | 単独タイプ（4種） | `knowledge` / `skill` / `experience` / `environment` |
| 0分類（最高スコアが40〜59点） | 最高スコアの単独タイプ | （同上） |
...
|------|-------|-------------|
| トップ | 運転不安診断 \| Drive Personal Lab | ペーパードライバー・運転初心者向け。知識・技術・経験・環境の4分類で運転不安をセルフ診断。あなたのタイプを無料で診断。 |
| 診断フロー | 診断中 \| Drive Personal Lab | 16問の質問に答えて、あなたの運転不安タイプを診断中。 |
| 結果（動的） | [タイプ名] \| Drive Personal Lab | あなたの運転不安タイプは[タイプ名]です。次の小さな一歩を一緒に考えましょう。 |
| 前回の結果 | 前回の診断結果 \| Drive Personal Lab | 前回の運転不安診断の結果を振り返る。 |
