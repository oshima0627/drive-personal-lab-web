...
import { buildAdviceLineShareUrl } from '@/lib/shareUtils';
...
export default function AdviceChecklist({ typeId, checkedIds }: AdviceChecklistProps) {
  ...
  const handleShareChecked = () => {
    ...
    const url = buildAdviceLineShareUrl(checkedTexts);
    ...
    <div>
      ...
      {checkedIds.length > 0 && (
        <button
          ...
          className="w-full py-3 px-4 bg-[#06C755] text-white rounded-xl text-sm font-medium hover:bg-[#05b04b] transition-colors flex items-center justify-center gap-2"
        >
          <svg
